...
function getSubEnv() {
  try {
    if ((typeof localStorage === "undefined" ? "undefined" : _typeof(localStorage)) === 'object' && !!window.localStorage && typeof localStorage.getItem === 'function') {
      return window.localStorage.getItem(_constant2.SUB_ENV_KEY);
    }
  } catch (err) {
    return null;
  }
...
function getForceHost() {
  try {
    return (0, _cookie.getCookie)(_constant2.DEV_FORCE_HOST_KEY);
  } catch (err) {
    return null;
  }
}
...
function getResponseDataType(header) {
  ...
  return _constant.MIME_TYPE[mime];
}
function trim(str) {
...
function _ajax(options) {
  ...
  var done = function done(errOrEvent, responseData) {
    ...
    if (errOrEvent) {
      var requestError = (0, _errors.createRequestError)((0, _utils2.transformEventToError)(errOrEvent), options, {
        status: xhr.status,
  ...
  var handleLoad = function handleLoad() {
    if ((0, _utils.validStatus)(xhr.status, protocol)) {
      try {
  ...
  if (isDevOrFatEnv) {
    ...
    if (subEnv) {
      params[_constant2.SUB_ENV_QUERY_NAME] = subEnv;
    }
  }
  var traceId = "".concat(getVid() || getRandom(10), "-").concat(new Date().getTime(), "-").concat(getRandom(10));
  params[_constant2.TRACE_ID] = traceId;
  if (params) {
    url += (url.indexOf('?') > -1 ? '&' : '?') + (0, _utils.buildQuery)(params);
  }
  ...
  xhr.ontimeout = function () {
    return handleError(new _errors.TimeoutError(timeout));
  };
  xhr.onabort = function () {
    // mark user manual abort error
    if (xhr.__abortByUser) {
      handleError(_errors.AbortError.MANUAL_ABORT_ERROR());
    } else {
      handleError(new _errors.AbortError());
    }
  };
  xhr.open(method, url, true, null, null);
  var locale = _browser.default.getLocale();
  var currency = _browser.default.getCurrency();
  if (locale) xhr.setRequestHeader(_constant2.HEADER_LOCALE, locale);
  if (currency) xhr.setRequestHeader(_constant2.HEADER_CURRENCY, currency);
  xhr.setRequestHeader(_constant2.TRACE_ID, traceId);
  xhr.setRequestHeader(_constant2.TRIP_TRACE_ID, traceId);
...
var _request = function _request(opt) {
  if (opt.cache) {
    ...
    if (cacheResult && (0, _utils.isAbortable)(cacheResult)) {
      return cacheResult;
    }
  }
  var _xhr;
  ...
  var requestPromise = promise.then(function (result) {
    var response = result.response,
      url = result.url,
      xhr = result.xhr;
    (0, _track.trackAjaxSuccess)(response, {
      url: url
    }, xhr);
    ...
    var requestError = (0, _errors.createRequestError)(err, opt);
    if (opt.trackError !== false) {
      (0, _track.trackAjaxFail)(err, opt, _xhr);
    }
    if (_defaultErrorHandler.defaultErrorHandler) {
      (0, _defaultErrorHandler.defaultErrorHandler)(requestError);
    } else {
      throw requestError;
    }
  });
...
var request = exports.request = (0, _helper.bindRequestMethods)(_request);