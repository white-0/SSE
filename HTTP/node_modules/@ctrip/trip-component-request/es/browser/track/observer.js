...
import { trackAjaxTiming } from './metric';
import { PERFORMANCE_ENTRY_RESOURCE, XHR_INITIATOR_TYPE } from './constant';
var createAjaxPerformanceCallback = function createAjaxPerformanceCallback() {
  var filters = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : [];
  return function (entries) {
    var resourceEntries = entries.getEntriesByType(PERFORMANCE_ENTRY_RESOURCE);
    var _iterator = _createForOfIteratorHelper(resourceEntries),
      _step;
    try {
      for (_iterator.s(); !(_step = _iterator.n()).done;) {
        var _entry = _step.value;
        if (_entry.initiatorType !== XHR_INITIATOR_TYPE) {
          continue;
        }
        if (filters.length) {
...
export var createAjaxPerformanceObserver = function createAjaxPerformanceObserver() {
  var filters = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : [];
  var observer = new PerformanceObserver(createAjaxPerformanceCallback(filters));
  observer.observe({
    entryTypes: [PERFORMANCE_ENTRY_RESOURCE]
  });
  return observer;
};
export function initGlobalAjaxPerformanceObserver() {
