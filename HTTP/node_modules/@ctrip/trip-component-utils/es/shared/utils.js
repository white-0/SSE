...
import { LOCALE_MAP_SITE } from './sitemap';
import { buildQuery, parseQuery } from './url-helper';
...
export function formatTripEnv(appEnv) {
  switch (appEnv) {
    case 'dev':
    case 'fat':
    case 'fws':
 ...
 * @param locale
 */
export function getSiteByLocale(locale) {
  var formattedLocale = formatLocale(locale, 'aa_bb');
  return LOCALE_MAP_SITE[formattedLocale] || formattedLocale.split('_')[1].toUpperCase();
}
export function formatUrlWithTripContext(url, tripContext, opt) {
  if (!url) return '';
  ...
  try {
    ...
    var originQuery = (urlQuery ? parseQuery(urlQuery, true) : {}) || {};
    ...
    var newUrl = "".concat(urlBase || '', "?").concat(buildQuery(originQuery)).concat(urlHash ? "#".concat(urlHash) : '');
    return newUrl;
  } catch (e) {
