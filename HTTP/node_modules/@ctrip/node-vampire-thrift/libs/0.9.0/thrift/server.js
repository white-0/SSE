...
const ttransport = require('./transport');
// const BinaryParser = require('./binary_parser').BinaryParser;
const TBinaryProtocol = require('./protocol').TBinaryProtocol;

exports.createServer = function(cls, handler, options) {
    if (cls.Processor) {
        cls = cls.Processor;
    }
    let processor = new cls(handler);
    ...
    return net.createServer(function(stream) {
        stream.on('data', transport.receiver(function(transport_with_data) {
            let input = new protocol(transport_with_data);
            let output = new protocol(new transport(undefined, function(buf) {
                stream.write(buf);
            }));
