...
let Connection = exports.Connection = function(stream, options) {
    let self = this;
    ...
    this.connection.addListener('data', self.transport.receiver(function(transport_with_data) {
        let message = new self.protocol(transport_with_data);
        try {
            let header = message.readMessageBegin();
            let dummy_seqid = header.rseqid * -1;
            let client = self.client;
            client._reqs[dummy_seqid] = function(err, success) {
                transport_with_data.commitPosition();

                let callback = client._reqs[header.rseqid];
...
exports.createConnection = function(host, port, options) {
    ...
    let connection = new Connection(stream, options);
...
exports.createClient = function(cls, connection) {
    ...
    let client = new cls(new connection.transport(undefined, function(buf) {
    ...
    client._timer = {};
    client._reqs = new Proxy(client._reqs, {
        get: function(target, key) {
            return target[key];
        },
        set: function(target, key, value) {
            target[key] = value;
            client._timer[key] = setTimeout(() => {
                try {
                    target[key](new Error('Response Timeout'));
                } catch (e) {
                    // empty
                }
                delete target[key];
                delete client._timer[key];
            }, key.startsWith('-') ? 5000 : 8000);
            return value;
        },
        deleteProperty: function(target, key) {
            if (client._timer[key]) {
                clearTimeout(client._timer[key]);
                delete client._timer[key];
            }
            delete target[key];
        }
...
let StdIOConnection = exports.StdIOConnection = function(command, options) {
    let command_parts = command.split(' ');
    command = command_parts[0];
    let args = command_parts.splice(1, command_parts.length - 1);
    let child = this.child = child_process.spawn(command, args);
