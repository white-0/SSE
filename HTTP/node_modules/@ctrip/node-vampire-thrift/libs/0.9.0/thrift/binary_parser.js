...
let p = exports.BinaryParser = function(bigEndian, allowExceptions) {
    this.bigEndian = bigEndian || true;
    this.allowExceptions = allowExceptions;
};
p.bigEndian = true;

let IBuffer = exports.BinaryParser.IBuffer = function(bigEndian, buffer) {
...
IBuffer.prototype.checkBuffer = function(neededBits) {
    if (!this.hasNeededBits(neededBits)) {
        console.log('[ Thrift ] Missing: ' + neededBits + ' ' + this.buffer.length);
...
p.decodeFloat = function(data, precisionBits, exponentBits) {
    let b = new this.IBuffer(this.bigEndian, data);
...
p.decodeInt = function(data, bits, signed, forceBigEndian) {
    let b = new this.IBuffer(this.bigEndian || forceBigEndian, data),
        x = b.readBits(0, bits),
        max = Math.pow(2, bits);
...
p.encodeFloat = function(data, precisionBits, exponentBits) {
    ...
    return Buffer.from(this.bigEndian ? r.reverse() : r);
};
...
p.encodeInt = function(data, bits, signed, forceBigEndian) {
    ...
    data < 0 && (data += max);
    let r = [];
    for (; data; r[r.length] = data % 256, data = Math.floor(data / 256));
    for (bits = -(-bits >> 3) - r.length; bits--; r[r.length] = 0);
    return Buffer.from((this.bigEndian || forceBigEndian) ? r.reverse() : r);
};
...
p.hprint = function(s) {
    for (let i = 0; i < s.length; i++) {
        if (s.charCodeAt(i) < 32) {
            let number = s.charCodeAt(i) <= 15 ? '0' + s.charCodeAt(i).toString(16) : s.charCodeAt(i).toString(16);
            util.debug(number + ' : ');
        } else {
            let number = s.charCodeAt(i) <= 15 ? '0' + s.charCodeAt(i).toString(16) : s.charCodeAt(i).toString(16);
