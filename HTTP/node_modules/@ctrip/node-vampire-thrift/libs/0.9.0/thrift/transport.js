...
let InputBufferUnderrunError = exports.InputBufferUnderrunError = function() {};

let TFramedTransport = exports.TFramedTransport = function(buffer, callback) {
...
TFramedTransport.receiver = function(callback) {
    ...
    return function(data) {
        ...
        if (residual) {
            let dat = Buffer.alloc(data.length + residual.length);
...
TFramedTransport.prototype = {
    ...
    write: function(buf, encoding) {
        if (typeof(buf) === 'string') {
            ...
            buf = Buffer.from(buf, encoding || 'ascii');
        }
    ...
    flush: function() {
        let out = Buffer.alloc(this.outCount),
            pos = 0;
        ...
        if (this.onFlush) {
            ...
            let msg = Buffer.alloc(out.length + 4);
...
let TBufferedTransport = exports.TBufferedTransport = function(buffer, callback) {
...
TBufferedTransport.receiver = function(callback) {
    ...
    return function(data) {
        if (reader.writeCursor + data.length > reader.inBuf.length) {
            let buf = Buffer.alloc(reader.writeCursor + data.length);
...
TBufferedTransport.prototype = {
    commitPosition: function() {
        let unreadedSize = this.writeCursor - this.readCursor;
        let bufSize = (unreadedSize * 2 > this.defaultReadBufferSize) ? unreadedSize * 2 : this.defaultReadBufferSize;
    ...
    readAll: function() {
        ...
        let buf = Buffer.alloc(this.writeCursor - this.readCursor);
        this.inBuf.copy(buf, 0, this.readCursor, this.writeCursor);
    ...
    flush: function() {
        ...
        let msg = Buffer.alloc(this.outCount),
            pos = 0;
        this.outBuffers.forEach(function(buf) {
            buf.copy(msg, pos, 0);
            pos += buf.length;
