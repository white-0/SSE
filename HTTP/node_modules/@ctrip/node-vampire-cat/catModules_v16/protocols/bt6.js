...
function createRaw(item) {
    ...
    let head = Buffer.concat([
        ...
        item.__outerIds ? createVarString(item.__outerIds['x-cat-parent-id']) : util.varint(0), // Parent Message ID
        item.__outerIds ? createVarString(item.__outerIds['x-cat-root-id']) : util.varint(0), // Root Message ID
        util.varint(0), // Session Token
...
function createItemRaw(_context, item) {
    ...
    if (item.__type == 'event' || item.__type == 'heartbeat' || item.__type == 'error') {
        body = Buffer.concat([
            ...
            util.varint(item.__startTime - _context.baseline),
