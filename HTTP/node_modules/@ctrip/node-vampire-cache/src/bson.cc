        ...
        else if (value->IsObject())
        {
            ...
            if (value->IsArray())
            {
                ...
                *reinterpret_cast<uint32_t *>(current) = len;
                ...
                for (uint32_t i = 0; i < len; i++)
                {
                    ...
                    write(arr->Get(Nan::GetCurrentContext(),i).ToLocalChecked());
                }
            }
            ...
            { // TODO: support for other object types
                ...
                *reinterpret_cast<uint32_t *>(current) = len;
                ...
                for (uint32_t i = 0; i < len; i++)
                {
                    Local<Value> name = names->Get(Nan::GetCurrentContext(),i).ToLocalChecked();
                    write(name);
                    write(obj->Get(Nan::GetCurrentContext(),name).ToLocalChecked());
                }
            }
        }
        else
...
static v8::Local<v8::Value> parse(const uint8_t *&data, object_wrapper_t *&objects)
{
    ...
    case bson::Array:
        ...
        data += sizeof(uint32_t);
        {
            ...
            for (uint32_t i = 0; i < len; i++)
            {
                (void)arr->Set(Nan::GetCurrentContext(),i, parse(data, objects));
            }
            return arr;
        }
    case bson::Object:
        ...
        data += sizeof(uint32_t);
        {
            ...
            for (uint32_t i = 0; i < len; i++)
            {
                ...
                (void)obj->Set(Nan::GetCurrentContext(),name, parse(data, objects));
            }
            return obj;
        }
    case bson::ObjectRef:
